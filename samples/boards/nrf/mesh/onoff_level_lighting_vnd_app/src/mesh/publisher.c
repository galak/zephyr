...
void publish(struct k_work *work)
{
	...
	if (gpio_pin_get(button_device[0], DT_GPIO_PIN(DT_ALIAS(sw0), gpios)) == 1) {
#if defined(ONOFF)
...
#endif
	} else if (gpio_pin_get(button_device[1], DT_GPIO_PIN(DT_ALIAS(sw1), gpios)) ==
		   1) {
#if defined(ONOFF)
		bt_mesh_model_msg_init(root_models[3].pub->msg,
				       BT_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK);
		...
		err = bt_mesh_model_publish(&vnd_models[0]);
#endif
	} else if (gpio_pin_get(button_device[2], DT_GPIO_PIN(DT_ALIAS(sw2), gpios)) ==
		   1) {
		...
		err = bt_mesh_model_publish(&root_models[16]);
#endif
	} else if (gpio_pin_get(button_device[3], DT_GPIO_PIN(DT_ALIAS(sw3), gpios)) ==
		   1) {
#if defined(GENERIC_LEVEL)
		bt_mesh_model_msg_init(root_models[5].pub->msg,
				       BT_MESH_MODEL_OP_GEN_LEVEL_SET_UNACK);
