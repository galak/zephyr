...
also use QEMU to emulate the AN547 platform running on the MPS3.

More information about the board can be found at the `MPS3 FPGA Website`_.

.. note::
   This board configuration makes no claims about its suitability for use
   with actual MPS3 hardware systems using AN547, or any other hardware
...
The mps3_an547 board provides the following user push buttons:
...
MPS3 AN547 is a Cortex-M33 based SoC and has 15 fixed exceptions and 77 IRQs.
...
The ARM MPS3 AN547 Board has 4 CMSDK AHB GPIO controllers. Each providing 16
...
Mapping from the ARM MPS3 AN547 Board pins to GPIO controllers:
...
MPS3 has 8 built-in LEDs connected to Serial Configuration Controller (SCC).
...
MPS3 AN547 has several clocks connected:
...
The MPS3 AN547 has five UARTs. The Zephyr console output by default, uses
...
The MPS3 AN547 implements a Serial Configuration Control (SCC) register.
...
MPS3 AN547 supports the v8m security extension, and by default boots to the
secure state.
...
- A Serial Port which is J10 on MPS3 board
...
for the AN547 target while replacing the platform with
``-DTARGET_PLATFORM=AN547`` and the compiler (if required) with
``-DCOMPILER=GNUARM``.
...
USB connection exposing a Mass Storage (``V2M_MPS3`` by default).

The update requires 3 steps:

1. Copy application files to ``<MPS3 device name>/SOFTWARE/``.
2. Open ``<MPS3 device name>/MB/HBI0263C/AN547/images.txt``.
...
.. _MPS3 AN547 Technical Reference Manual (TRM):
   http://infocenter.arm.com/help/topic/com.arm.doc.dai0547c/DAI0547C_Example_SSE200_Subsystem_for_MPS3plus.pdf
