...
class EDTScheck(EDTSDatabase):
    ...
    def expand(self, key, value):
        if isinstance(value, dict):
            return [ (key + '/' + k, v) for k, v in self.discover_device(value).items() ]
        else:
            return [ (key, value) ]


    def discover_device(self, device_dict):
        items = [ item for k, v in device_dict.items() for item in self.expand(k, v) ]
        return dict(items)
    ...
    def main(self):
        ...
        for dev_id in ref_devices:
            ...
            flatten_ref_device = self.discover_device(ref_device)
            flattend_device_to_check = self.discover_device(device_to_check)
            device_list_message += self.compare_lists(flattend_device_to_check,
                                                 flatten_ref_device,
                                                 "%s parameters" % dev_id)
            if device_list_message is not '':
                logger.error(device_list_message)
                return
            # Check device properties values
            for prop in flatten_ref_device:
                value = self._edtstocheck.get_device_property(dev_id, prop, '')
