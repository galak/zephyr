...
class DTFlash(DTDirective):
    ...
    def _extract_code_partition(self, node_address, yaml, prop, def_label):
        ...
        if node and node is not self._flash_node:
            ...
            load_defs['CONFIG_ZEPHYR_CODE_PARTITION_OFFSET'] = load_offset
            load_size = node['props']['reg'][1]
            load_defs['CONFIG_ZEPHYR_CODE_PARTITION_SIZE'] = load_size
        else:
            load_defs['CONFIG_ZEPHYR_CODE_PARTITION_OFFSET'] = 0
            load_defs['CONFIG_ZEPHYR_CODE_PARTITION_SIZE'] = 0
